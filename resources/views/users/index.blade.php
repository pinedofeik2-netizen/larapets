...
@section('content')
    ...
    <div class="flex flex-col gap-4 justify-center items-center">
        ...
        <label class="input text-white bg-[#0009] md:w-110 w-57 outline mb-10 outline-white">
            ...
            <input type="search" placeholder="Search users..." name="qsearch" id="qsearch" />
        </label>
    </div>
...
<script>
    ...
    // Búsqueda en tiempo real - VERSIÓN SIMPLE
    let timeout;
        ...
        if(query.length >= 2) {
            timeout = setTimeout(function() {
                $.ajax({
                    ...
                    beforeSend: function() {
                        $('#results-container').html('<tr><td colspan="7" class="text-center py-8"><div class="loading loading-spinner loading-md"></div> Searching...</td></tr>');
                    },
                    success: function(response) {
                        $('#results-container').html(response);
                    },
                    error: function() {
                        $('#results-container').html('<tr><td colspan="7" class="text-center py-8 text-red-500">Error searching</td></tr>');
                    }
                });
            }, 500);
